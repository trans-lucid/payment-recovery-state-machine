...
describe("hidden timeout reconciliation", () => {
  it("recovers a gateway timeout after the gateway later reports captured", async () => {
    ...
    const created = await service.createPayment({
      ...
      amountCents: 7300,
      currency: "USD",
      idempotencyKey: "hidden-timeout-key"
    });

    if (created.intent.state !== "pending_reconciliation") {
    ...
    if (reconciled?.state !== "captured" || !reconciled.gatewayEvidence.gatewayLookup) {
