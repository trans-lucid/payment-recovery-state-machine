...
import { fulfillmentForState, stateFromGateway } from "./paymentState";
...
export async function processReconciliationOnce(
  ...
  for (const message of messages) {
    ...
    if (intent) {
      const gatewayResult = await gateway.getChargeByIdempotencyKey(message.body.idempotencyKey);
      if (!gatewayResult) {
        await store.updateIntent(intent.id, {
          state: "pending_reconciliation",
          gatewayEvidence: {
            ...intent.gatewayEvidence,
            reconciliation: "gateway truth unavailable"
          },
      ...
      const state = stateFromGateway(gatewayResult);
      await store.updateIntent(intent.id, {
        state,
        ...
        gatewayEvidence: {
          ...intent.gatewayEvidence,
          gatewayLookup: gatewayResult.evidence,
          reconciledAt: new Date().toISOString()
        },
        fulfillmentStatus: fulfillmentForState(state)
      });
      processed += 1;
    }
